...
// Handle Add Device (Admin)
if (isset($_POST['add_device'])) {
    $owner_id = mysqli_real_escape_string($koneksi, $_POST['owner_id']);
    ...
    $query = "INSERT INTO device (user_id, device_name, broker_url, mq_user, mq_pass, device_type, broker_port)
              VALUES ('$owner_id', '$dev_name', '$broker', '$user_mq', '$pass_mq', '$dev_type', '$broker_port')";
...
if (isset($_POST['edit_device'])) {
    ...
    if (mysqli_query($koneksi, $query_update)) {
        ...
        $_SESSION['toast'] = ['type' => 'success', 'message' => 'Device updated successfully!'];
    } else {
        $_SESSION['toast'] = ['type' => 'error', 'message' => 'Failed to update device: ' . mysqli_error($koneksi)];
    }
    header("Location: devices.php");
    exit;
}

// Handle Delete Device (Admin)
if (isset($_POST['delete_device'])) {
    $id_device = mysqli_real_escape_string($koneksi, $_POST['delete_device_id']);
    $res = mysqli_query($koneksi, "SELECT device_name FROM device WHERE device_id = '$id_device'");
    $dev = mysqli_fetch_assoc($res);
    $dev_name = $dev['device_name'] ?? 'Unknown';

    if (mysqli_query($koneksi, "DELETE FROM device WHERE device_id = '$id_device'")) {
        insertAdminAuditLog($koneksi, $admin_id, 'delete_device', 'device', $id_device, ['name' => $dev_name]);
...
$users_dropdown = mysqli_query($koneksi, "SELECT user_id, user_name FROM user ORDER BY user_name ASC");
$users_list = [];
while($u = mysqli_fetch_assoc($users_dropdown)) $users_list[] = $u;
...
<div class="max-w-7xl mx-auto px-6 py-10">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        ...
        <div class="flex gap-4">
            <button onclick="openAddDeviceModal()" class="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-900/10 transition flex items-center gap-2">
                            ...
                            <td class="px-6 py-4 text-right space-x-1">
                                <div class="flex justify-end gap-1">
                                    ...
                                    <form method="POST" onsubmit="return confirm('Delete this device permanently?');" class="inline">
                                        <input type="hidden" name="delete_device_id" value="<?= $device['device_id'] ?>">
